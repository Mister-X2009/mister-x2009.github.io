...
async function saveProduct(){
  ...
  const qty = Number(els.qty.value)||0, price=Number(els.price.value)||0;
  const barcode = els.barcode.value.trim() || generateBarcode(), category=els.category.value.trim()||'Unkategorisiert';
  let imgData=''; if(els.img.files && els.img.files[0]) imgData=await toBase64(els.img.files[0]);
  const existing=db.find(p=>p.barcode===barcode);
  if(existing){ existing.name=name; existing.qty=qty; existing.price=price; existing.category=category; if(imgData) existing.img=imgData; }
  else db.push({id:Date.now(), name, qty, price, barcode, category, img:imgData, sold:0});
...
function parseCSV(text){
  const rows=[], cur=[], inQuotes=false; let curCell='', i=0, inQ=false;
  for(i=0;i<text.length;i++){
    const ch=text[i], nxt=text[i+1];
    if(inQ){
      if(ch==='"'){ if(nxt==='"'){ curCell+='"'; i++; } else inQ=false; } 
      else curCell+=ch;
    } else {
      if(ch==='"') inQ=true;
      else if(ch===','){ cur.push(curCell); curCell=''; }
      else if(ch==='\r') continue;
      else if(ch==='\n'){ cur.push(curCell); rows.push(cur.slice()); cur=[]; curCell=''; }
      else curCell+=ch;
    }
  }
  if(curCell!=='' || cur.length>0){ cur.push(curCell); rows.push(cur.slice()); }
  return rows;
}
...
function renderProducts(){
  ...
  db.filter(p=>{
    ...
    node.querySelector('.edit').addEventListener('click', ()=>{
      els.name.value = p.name; els.barcode.value = p.barcode; els.qty.value = p.qty; els.price.value = p.price; els.category.value = p.category; window.scrollTo({top:0, behavior:'smooth'});
    });
...
function renderStats(){
  ...
  const last30={}; const now=new Date();
  for(let i=0;i<30;i++){ const d=new Date(now); d.setDate(now.getDate()-i); last30[dateKey(d)]=0; }
...
function sellProduct(){
  ...
  prod.qty-=qty; prod.sold=(prod.sold||0)+qty; const sum=qty*Number(prod.price||0);
...
function exportCSV(){
  ...
  const a=document.createElement('a'); a.href=url; a.download='produkte_export.csv'; a.click(); URL.revokeObjectURL(url);
}

async function importCSV(){
  const file=els.csvFile.files[0]; if(!file) return alert('Keine Datei gewählt');
  const txt=await file.text(); const lines=parseCSV(txt); if(lines.length<2) return alert('Keine Daten');
  const headers=lines[0].map(h=>h.toLowerCase()); const data=lines.slice(1).map(r=>Object.fromEntries(r.map((c,i)=>[headers[i]||i,c])));
...
// Labels
function createLabelHTML(prod){ return `<div class="labelPage"><div class="labelInner"><h2>${escapeHtml(prod.name)}</h2><p>Preis: ${Number(prod.price||0).toFixed(2)} €</p><svg id="lbl-${prod.id}"></svg><p>${escapeHtml(prod.barcode)}</p></div></div>`; }
function printLabels(all){
  let items=[];
  if(all) items=db.slice();
  else { const bc=els.labelSelect.value; const p=db.find(x=>x.barcode===bc); if(!p) return alert('Kein Produkt gewählt'); items=[p]; }
